...
class App(QWidget):
    def __init__(self, prop_net, fuse_net, s2m_ctrl:S2MController, fbrs_ctrl:FBRSController, 
                    images, masks, num_objects, mem_freq, mem_profile, output_dir):
        super().__init__()
    ...
    def save(self):
        ...
        # G:\Mixed_Reality\mr_dataset\dataset_test/polycam_firstAidkit
        folder_path = self.output_dir   
        self.console_push_text('Saving masks...')
        mask_dir = path.join(folder_path, 'masks_2')
        ...
        for i in range(self.num_frames):
            ...
            modified_mask = Image.fromarray(mask_data)
            modified_mask = ImageOps.invert(modified_mask)
            #modified_mask.save(os.path.join(mask_dir, '{:05d}.png'.format(i)))
            modified_mask.save(os.path.join(mask_dir, "frame_{:05d}.png".format(i+1)))

            
            # Save overlay
            #overlay = overlay_davis(self.images[i], self.current_mask[i]) 
            #overlay = Image.fromarray(overlay)
            #overlay = Image.fromarray(self.images[i])
            #overlay.save(os.path.join(overlay_dir, "frame_{:05d}.png".format(i+1)))
...
if __name__ == '__main__':
    ...
    with torch.cuda.amp.autocast(enabled=not args.no_amp):
        ...
        ex = App(prop_model, fusion_model, s2m_controller, fbrs_controller, 
                    images, masks, num_objects, args.mem_freq, args.mem_profile, args.output_dir)
        sys.exit(app.exec_())
